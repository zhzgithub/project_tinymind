...
if __name__ == '__main__':
    ...
    data_path = '/media/zhz/6cff368c-1c54-4cba-aeff-afb6309a2a9a/ctw-baseline/data/gray_char_images_/'  # 原始数据存放地址
    data_tra_path = '/media/zhz/6cff368c-1c54-4cba-aeff-afb6309a2a9a/ctw-baseline/data/new_train_data_/'  # new_train_data新取的名
    data_val_path = '/media/zhz/6cff368c-1c54-4cba-aeff-afb6309a2a9a/ctw-baseline/data/new_val_data_/'
    ...
    for i in full_child_file:			# i遍历每个汉字类别路径所组成的full_child_file列表
        ...
        move_ratio(train_list, 'gray_char_images_', 'new_train_data_')	# train_list是某个汉字文件夹的前360张图片路径列表。
																		# 把原来train文件夹下的某类别汉字的前360张图片复制到new_train_val_data/font_tra路径下
        move_ratio(val_list, 'gray_char_images_', 'new_val_data_')		# val_list  是某个汉字文件夹的后 40张图片路径列表
